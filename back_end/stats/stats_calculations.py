...
from stats.helper_functions import (calculate_per_game_stat, validate_average_stat_data, validate_percent_stat_data)


# NOTE: Function to calculate points per game
def calculate_ppg(data):
    if len(data) == 2:
        total_points, games_played = validate_average_stat_data(data, "PPG")
...
def calculate_rpg(data):
    if len(data) == 2:
        total_rebounds, games_played = validate_average_stat_data(data, "RPG")
...
def calculate_apg(data):
    if len(data) == 2:
        total_assists, games_played = validate_average_stat_data(data, "APG")
...
def calculate_spg(data):
    if len(data) == 2:
        total_steals, games_played = validate_average_stat_data(data, "SPG")
...
def calculate_bpg(data):
    if len(data) == 2:
        total_blocks, games_played = validate_average_stat_data(data, "BPG")
...
def calculate_tov(data):
    if len(data) == 2:
        total_turnovers, games_played = validate_average_stat_data(data, "TOV")
        return calculate_per_game_stat(total_turnovers, games_played)


def calculate_fg_percent(data):
    fg_pct = validate_percent_stat_data(data, "FG%")
    return fg_pct


def calculate_three_point_percent(data):
    three_point_pct = validate_percent_stat_data(data, "3P%")
    return three_point_pct


def calculate_ft_percent(data):
    ft_percent = validate_percent_stat_data(data, "FT%")
    return ft_percent
